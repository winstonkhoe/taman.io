                    ...
                    <span class="d-flex flex-column flex-grow-1">
                        <div class="w-100 h-25 d-flex align-items-center">
                            ...
                            <p class="m-0 ms-3 p-0" style="font-size: 1rem; font-weight: 400;">
                                {{ \Carbon\Carbon::parse($data[$i]->created_at)->setTimezone('Asia/Jakarta')->format('d M Y, H:i') }}
                            </p>
                            {{-- <p>{{$data[$i]->user}}</p> --}}
                        </div>
                        <div class="w-100 h-75 mt-3" style="">
                            ...
                            @if ($data[$i]->garden)
                                <div class="w-100 h-25 d-flex justify-content-between mt-4">
                                    <img src={{ Storage::url('public/uploads/' . $data[$i]->garden[0]->offer->design->image->filename) }}
                                        style="width: 250px; height: 150px; object-fit: cover" alt="">
                                    <div class="w-100 ms-3 d-flex flex-column justify-content-start">
                                        <h6 class="m-0 p-0 mb-3" style="font-size: 1.5rem; font-weight: 600">
                                            @foreach ($data[$i]->garden as $garden)
                                                {{ ' ' . $garden->offer->design->name }}
                                            @endforeach
                                            {{-- {{$data[$i]->garden[0]->offer->design->name}} --}}
                                        </h6>
                                        <div class="d-flex justify-content-between" style="width: 250px">
                                            <p class="m-0 p-0" style="font-size: 0.9rem; font-weight: 700; opacity: 0.6;">
                                                Design Price</p>
                                            <p class="m-0 p-0" style="font-size: 0.9rem; font-weight: 600; opacity: 0.6;">
                                                {{ 'Rp ' . number_format($data[$i]->garden[0]->offer->design->price) }}</p>
                                        </div>
                                        <div class="d-flex justify-content-between" style="width: 250px">
                                            <p class="m-0 p-0" style="font-size: 0.9rem; font-weight: 700; opacity: 0.6;">
                                                Construction Price</p>
                                            <p class="m-0 p-0" style="font-size: 0.9rem; font-weight: 600; opacity: 0.6;">
                                                {{ 'Rp ' . number_format($data[$i]->garden[0]->offer->price) }}</p>
                                        </div>
                                        <hr class="my-2 w-50">
                                        <div class="d-flex flex-column">
                                            <p class="m-0 p-0" style="font-size: 0.9rem; font-weight: 700; opacity: 0.8;">
                                                Total</p>
                                            <h3 class="m-0 p-0" style="font-size: 1.4rem; font-weight: 700;">
                                                {{ 'Rp ' . number_format($data[$i]->garden[0]->offer->design->price + $data[$i]->garden[0]->offer->price) }}
                                            </h3>
                                        </div>
                                    </div>
                                </div>
