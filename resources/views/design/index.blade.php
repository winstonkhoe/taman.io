    ...
    <form action="" action="" method="POST">
        ...
        <div class="container-fluid d-flex flex-column px-5 background min-vh-100 pt-5" style="">
            ...
            <div class="w-50" style="height: 300px">
                @if ($data->image)
                    <img class="w-100 h-100 br-card" src={{ Storage::url('public/uploads/' . $data->image->filename) }}
                        alt="Design" style="object-fit: cover; object-position: center">
                @endif
            </div>
                                    ...
                                    <div class="col-4">
                                        ...
                                        <label class="constructor-option w-100 h-100">
                                            <input type="radio" name="offer_id"
                                                id={{ $data->contractors[$j]->seller->id }}
                                                value={{ $data->contractors[$j]->id }}>
                                            <div class="container br-card d-flex align-items-center p-3 mt-4"
                                                onclick="setPrice(`{{ number_format($data->contractors[$j]->price) }}`, `{{ number_format($data->price + $data->contractors[$j]->price) }}`)">
                                                <div class="" style="height: 50px; width: 50px">
                                                    <img class="w-100 h-100 rounded-circle"
                                                        src={{ Storage::url('public/uploads/' . $data->contractors[$j]->seller->image) }}
                                                        alt="profile-picture"
                ...
                <div class="summary container br-card w-25 mb-5">
                    <div class="d-flex flex-column p-3">
                        ...
                        <div class="d-flex w-100 justify-content-between">
                            ...
                            <h6 id="construction-price" class="header" style="font-size: 1rem; font-weight: 400;">Rp
                                {{ number_format($constructor_price) }}</h6>
                        </div>
                        ...
                        <button class="mt-5" type="submit">Start Building</button>
                            @if ((auth()->user()->artist && auth()->user()->artist->id == $data->artist_id) ||
                                (auth()->user()->seller && in_array(auth()->user()->seller->id, $contractors)))
                                @if (auth()->user()->artist && auth()->user()->artist->id == $data->artist_id)
