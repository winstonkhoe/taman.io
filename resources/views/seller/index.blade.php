    ...
    <div class="row mb-5">
        ...
        @for ($i = 0; $i < count($data); $i++)
            <div class="product-card  ms-4 mt-4 p-0" style="width: 230px">
                <a class="d-flex flex-column flex-grow-1" href="/service/{{ $data[$i]->id }}">
                    <div class="w-100" style="height: 200px">
                        <img class="w-100 h-100"
                            src=@if ($data[$i]->image) {{ Storage::url('public/uploads/' . $data[$i]->image->filename) }}
                        @else
                            {{ asset('/assets/no_image.webp') }} @endif
                            alt="Service" style="object-fit: cover; object-position: center">
                    </div>
