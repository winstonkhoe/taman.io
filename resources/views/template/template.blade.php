...
<head>
    ...
    <link rel="icon" href="{{ asset('/assets/tamankhoe-light.png') }}">
        ...
        <div class="h-100 w-100">
            <div class="d-flex justify-content-center align-items-center">
                <div class="d-flex flex-column justify-content-center">
                    <span class="p-4">
                        <div
                            class="container rounded-3 w-100 d-flex flex-column justify-content-center align-items-center">
                            <div class="nav-item d-flex align-items-center">
                                <img class="rounded-circle" style="width: 30px; height: 30px;"
                                    src="@if (auth()->check()) {{ auth()->user()->image ? Storage::url('public/uploads/' . auth()->user()->image) : asset('/assets/blank_profile_picture.png') }} @else {{ asset('/assets/blank_profile_picture.png') }} @endif"
                                    alt="">
                            ...
                            @if (auth()->user())
                                <a href=@if (auth()->user()->artist) {{ '/artist' }}
                                @else
                                    {{ '/register/artist' }} @endif
                                    class="nav-item d-flex align-items-center">
                                    <img class="rounded-circle" style="width: 30px; height: 30px;"
                                        src="@if (auth()->check()) {{ auth()->user()->artist && auth()->user()->artist->image ? Storage::url('public/uploads/' . auth()->user()->artist->image) : asset('/assets/blank_profile_picture.png') }} @else {{ asset('/assets/blank_profile_picture.png') }} @endif"
                                        alt="">
                                    <h5 class="m-0 ms-3 p-0" style="width: 100px">
                                        @if (auth()->user()->artist)
                                ...
                                <a href=@if (auth()->user()->seller) {{ '/seller' }}
                                        @else
                                            {{ '/register/seller' }} @endif
                                    class="nav-item d-flex align-items-center">
                                    <img class="rounded-circle" style="width: 30px; height: 30px;"
                                        src="@if (auth()->check()) {{ auth()->user()->seller && auth()->user()->seller->image ? Storage::url('public/uploads/' . auth()->user()->seller->image) : asset('/assets/blank_profile_picture.png') }} @else {{ asset('/assets/blank_profile_picture.png') }} @endif"
                                        alt="">
                                    <h5 class="m-0 ms-3 p-0" style="width: 100px">
                                        @if (auth()->user()->seller)
                    ...
                    <div class="flex-shrink-0 w-100 ">
                        <a class="d-flex justify-content-center" href="/" name="not-nav">
                            <img class="w-50" src="{{ asset('/assets/tamankhoe.png') }}" alt="plantSop">
                        </a>
                    </div>
